"""
论文图表生成 - 辅助函数库 v3
覆盖：统计图表（SciencePlots）| 示意图（draw.io XML）| 三线表（booktabs LaTeX）
...
_NODE_STYLES = {
    "input":    {"fill": "#DAEEF8", "stroke": "#1A73E8", "shape": "box", "style": "filled,rounded"},
    "process":  {"fill": "#FEF3D0", "stroke": "#E37400", "shape": "box", "style": "filled,rounded"},
    "output":   {"fill": "#DCF0E2", "stroke": "#1E8E3E", "shape": "box", "style": "filled,rounded"},
    "decision": {"fill": "#FCE8E6", "stroke": "#C5221F", "shape": "diamond", "style": "filled"},
    "default":  {"fill": "#F4F4F4", "stroke": "#5F6368", "shape": "box", "style": "filled,rounded"},
}
...
# ============================================================
# 模块五：三线表 PNG 渲染（matplotlib Axes.table 引擎）
# ============================================================


def _measure_col_widths(df, ax, fontsize=7):
    """估算每列文本宽度（以 axes 坐标为单位），用于等宽/自适应列宽"""
    ...
    for c in range(len(df.columns)):
        texts = [str(df.columns[c])] + [str(df.iloc[r, c]) for r in range(len(df))]
        max_w = 0
        for t in texts:
            tb = ax.text(0, 0, t, fontsize=fontsize, fontfamily="sans-serif",
                         ha="center", va="center")
    ...
    total = sum(max_ws)
    if total == 0:
        return [1.0 / len(max_ws)] * len(max_ws)
    fig_w_pt = fig.get_size_inches()[0] * fig.dpi
    return [w / fig_w_pt for w in max_ws]  # 转 axes 比例


def df_to_table_png(df, caption="", column="single",
                    output_path="table.png"):
    """DataFrame -> 论文标准三线表 PNG
    完全手动渲染（ax.text + ax.hlines），不依赖 ax.table 的内部坐标。
    三线精确位置：顶线在表头顶部，栏目线在表头底部，底线在表身底部。
    """
    ...
    # 行高自适应：表头稍高，表身依行数调整
    row_h = 0.12 if nrows <= 5 else 0.10  # axes 坐标中的行高
    header_h = row_h * 1.3
    cap_h = 0.15 if caption else 0.03
    pad_b = 0.03
    tbl_h = cap_h + header_h + nrows * row_h + pad_b
    fig_h = max(tbl_h, 0.8)
    fig, ax = plt.subplots(figsize=(fig_w, fig_h))
    ax.set_xlim(0, 1); ax.set_ylim(0, 1)
    ax.axis("off")

    # 列宽：先自适应测量，再归一化到 [0.08, 0.50] 区间防止极端值
    col_widths = _measure_col_widths(df, ax)
    col_widths = [max(0.08, min(w, 0.50)) for w in col_widths]
    total_w = sum(col_widths)
    col_widths = [w / total_w for w in col_widths]

    # 表格整体在 axes 中居中，留左右边距
    margin_x = 0.02
    table_left = margin_x
    table_right = 1 - margin_x
    table_w = table_right - table_left
    col_starts = [table_left + sum(col_widths[:c]) * table_w for c in range(ncols)]
    col_ends = [table_left + sum(col_widths[:c+1]) * table_w for c in range(ncols)]
    ...
    for r in range(nrows):
        row_top = header_bot - r * row_h
        row_bot = row_top - row_h
        data_rows.append((row_top, row_bot))

    # === 画三条线（三线表核心）===
    top_rule_y = header_top
    ax.hlines(y=top_rule_y, xmin=table_left, xmax=table_right,
              color="black", linewidth=0.8, zorder=5)

    mid_rule_y = header_bot
    ax.hlines(y=mid_rule_y, xmin=table_left, xmax=table_right,
              color="black", linewidth=0.4, zorder=5)

    bot_rule_y = data_rows[-1][1]
    ax.hlines(y=bot_rule_y, xmin=table_left, xmax=table_right,
              color="black", linewidth=0.8, zorder=5)

    # === 写标题 ===
    if caption:
        ax.text(0.5, y_top + 0.01, caption, fontsize=8, ha="center", va="bottom",
                fontweight="bold", zorder=3, fontfamily="sans-serif")

    # === 写表头 ===
    for c in range(ncols):
        cx = (col_starts[c] + col_ends[c]) / 2
        cy = (header_top + header_bot) / 2
        ax.text(cx, cy, str(df.columns[c]), fontsize=7,
                ha="center", va="center", fontweight="bold",
                fontfamily="sans-serif", zorder=3)

    # === 写表身 ===
    for r in range(nrows):
        row_top, row_bot = data_rows[r]
        for c in range(ncols):
            cx = (col_starts[c] + col_ends[c]) / 2
            cy = (row_top + row_bot) / 2
            val = str(df.iloc[r, c])
            ax.text(cx, cy, val, fontsize=7,
                    ha="center", va="center",
                    fontfamily="sans-serif", zorder=3)
